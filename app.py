...
DATA_PATH      = "balanced_multiclass.csv"  # adjust if your csv is elsewhere
HER_MODELS_DIR = "web_app"                  # where trained .pkl files are saved
PLOTS_DIR      = "plots"                    # where training plots are saved
SEED           = 42
TEST_SIZE      = 0.2
...
def load_her_artifacts():
    ...
    # Models (add LR; MLP/LR need scaler)
...
meta          = arts.get("meta", {})
label_encoder = arts.get("label_encoder", None)   # for XGBoost predictions
scaler        = arts.get("scaler", None)          # for MLP/LR inputs

# Saved training-time test indices (ensures matching results)
...
with tab_results:
    ...
    if models:
        ...
        for name, mdl in models.items():
            ...
            # --- Reuse training-time test set if available ---
            if saved_test_idx is not None:
                ...
                # fallback: new random split (demo mode)
                _, X_te, _, y_te = train_test_split(
                    X_all, y_all, test_size=TEST_SIZE, random_state=SEED, stratify=y_all
                )

            # Scale for MLP & Logistic Regression (only test set for evaluation)
            ...
            # Map labels if needed (XGB uses LabelEncoder for y)
